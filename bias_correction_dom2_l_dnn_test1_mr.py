...
d_t_1='2017-12-31' ; d_t_2='2018-06-29' ; 
...
mr_input=pd.concat([mod_hour_day_1['mrio'],obs_hour_day_1['mrio'],bias_hour_day_1['mrio'],rmse_hour_avg_day1['mrio']],axis=1)
mr_input.columns=['mod','obs','bias','rmse']
mr_input['hour']=mod_hour_day_1.index.hour
mr_input.insert(0,'Date',(mod_hour_day_1.index.strftime('%m')).astype(int)) 
...
mr_input=mr_input[d_t_1:d_t_2]
mr_input=mr_input.dropna(how='any')
######################################################################################################

date_index=mr_input.index[::24] ; rmse_cor=[] ;mr_data_new_hour=[]
...
for ii in range(0,len(date_index_req)):
    ...
    dt_mr_input_1=mr_input[dte1:dte2] ;  
   
    if dt_mr_input_1.shape >= (48,6) :
        dt_mr_input=dt_mr_input_1 ;
    else:
        dte_t=(dt.datetime.strptime(dte.strftime('%Y%m%d %H:%M:%S'),'%Y%m%d %H:%M:%S')+dt.timedelta(days=-2)).strftime('%Y-%m-%d %H:%M:%S')
        dt_mr_input=mr_input[dte_t:dte2]


    mr_input_1=pd.concat([(dt_mr_input.iloc[0:24]).reset_index(drop=True),dt_mr_input['mod'].iloc[24:48].reset_index(drop=True)],axis=1)
    mr_target=dt_mr_input['obs'].iloc[24:48].reset_index(drop=True) 
...
#    tmp_input_1.columns=cols
    ...
    hour_avg_bias_1=(pd.concat([hour_avg_bias['mrio'][13:],hour_avg_bias['mrio'][1:13]],axis=0)).reset_index(drop=True)
    b_add_tmp=mr_input_1['mod']- hour_avg_bias_1
     
##############################################################################################################
    X=mr_input_1.dropna(how='any').reset_index(drop=True)  ; Y=mr_target.dropna(how='any').reset_index(drop=True)  ; 
...
############### loading and predict Linear DNN combined regressor
    epochs=2000 ;
    h_size1=100 ; h_size2=100 ;itrn=50
    #m_dir='mr_linear_dnn_bias-cor_full_'+str(itrn)+'.bsz_ep.'+str(epochs)+'_hu.'+str(h_size1)+'.'+str(h_size2)+'.1_Mn_2016_2017_winter'
    m_dir='mr_linear_dnn_bias-cor_full_'+str(itrn)+'.bsz_ep.'+str(epochs)+'_hu.'+str(h_size1)+'.'+str(h_size1)+'.'+str(h_size2)+'.1_Mn_2016_2017_full'
    #m_dir='tmp_linear_dnn_bias-cor_full_'+str(itrn)+'.bsz_ep.'+str(epochs)+'_hu.'+str(h_size1)+'.'+str(h_size1)+'.'+str(h_size1)+'.'+str(h_size2)+'.1_Mn_2016_2017_winter'
    print (m_dir)
...
################################################################################################################################# 
    ...
    mr_data_new=pd.concat([mr_input_1['mod'],pd.DataFrame((y_new_l_dnn)),pd.DataFrame((b_add_tmp)),Y,dt_mr_input['bias'].iloc[24:48].reset_index(drop=True),\
                 bias_new,bias_ba],axis=1)
    mr_data_new.columns=['mod','mod_cor','mod_cor_ba','obs','bias_mod','bias_cor','bias_ba']
    mr_data_new.index=dt_mr_input.index[24:48]
    mr_data_new.insert(0,'Date',mr_data_new.index)
    mr_data_new_hour.append(mr_data_new)
...
#####################################################################################################################################
mr_data_new_hour=pd.concat(mr_data_new_hour,axis=0)     
...
mr_data_new_hour_jan=mr_data_new_hour['2017-12-31 13':'2018-02-01 12']
hourt = pd.to_timedelta(mr_data_new_hour_jan.index.hour,  unit='H')
mf=int(mr_data_new_hour_jan.shape[0]/24.0)

uc_mr_rmse_hour_avg_day1_jan=(((mr_data_new_hour_jan['bias_mod']**2).groupby(hourt)).mean())**0.5
uc_mr_rmse_hour_avg_day1_jan=pd.concat([uc_mr_rmse_hour_avg_day1_jan[13:24],uc_mr_rmse_hour_avg_day1_jan[0:13]])
uc_mr_rmse_hour_avg_day1_jan=pd.concat([uc_mr_rmse_hour_avg_day1_jan]*mf)
uc_mr_rmse_hour_avg_day1_jan.index=mr_data_new_hour_jan.index

cor_mr_rmse_hour_avg_day1_jan=(((mr_data_new_hour_jan['bias_cor']**2).groupby(hourt)).mean())**0.5
cor_mr_rmse_hour_avg_day1_jan=pd.concat([cor_mr_rmse_hour_avg_day1_jan[13:24],cor_mr_rmse_hour_avg_day1_jan[0:13]])
cor_mr_rmse_hour_avg_day1_jan=pd.concat([cor_mr_rmse_hour_avg_day1_jan]*mf)
cor_mr_rmse_hour_avg_day1_jan.index=mr_data_new_hour_jan.index 

mr_data_new_hour_jan.insert(8,'uc_rmse',uc_mr_rmse_hour_avg_day1_jan)
mr_data_new_hour_jan.insert(9,'cor_rmse',cor_mr_rmse_hour_avg_day1_jan)

plot_bars_rmse_hw(mr_data_new_hour_jan.iloc[11:35,:],'Mixing_ratio','Hour_wise','RMSE','201801' )


rmse_cor_jan=rmse_cor['2018-01-01':'2018-01-31']
plot_bars_rmse(rmse_cor_jan,'Mixing_ratio','day_1','rmse',rmse_cor_jan.index.strftime('%Y%m')[0])
###############################################################################################################################################
mr_data_new_hour_feb=mr_data_new_hour['2018-01-31 13':'2018-03-01 12']
hourt = pd.to_timedelta(mr_data_new_hour_feb.index.hour,  unit='H')
mf=int(mr_data_new_hour_feb.shape[0]/24.0)

uc_mr_rmse_hour_avg_day1_feb=(((mr_data_new_hour_feb['bias_mod']**2).groupby(hourt)).mean())**0.5
uc_mr_rmse_hour_avg_day1_feb=pd.concat([uc_mr_rmse_hour_avg_day1_feb[13:24],uc_mr_rmse_hour_avg_day1_feb[0:13]])
uc_mr_rmse_hour_avg_day1_feb=pd.concat([uc_mr_rmse_hour_avg_day1_feb]*mf)
uc_mr_rmse_hour_avg_day1_feb.index=mr_data_new_hour_feb.index

cor_mr_rmse_hour_avg_day1_feb=(((mr_data_new_hour_feb['bias_cor']**2).groupby(hourt)).mean())**0.5
cor_mr_rmse_hour_avg_day1_feb=pd.concat([cor_mr_rmse_hour_avg_day1_feb[13:24],cor_mr_rmse_hour_avg_day1_feb[0:13]])
cor_mr_rmse_hour_avg_day1_feb=pd.concat([cor_mr_rmse_hour_avg_day1_feb]*mf)
cor_mr_rmse_hour_avg_day1_feb.index=mr_data_new_hour_feb.index 

mr_data_new_hour_feb.insert(8,'uc_rmse',uc_mr_rmse_hour_avg_day1_feb)
mr_data_new_hour_feb.insert(9,'cor_rmse',cor_mr_rmse_hour_avg_day1_feb)

plot_bars_rmse_hw(mr_data_new_hour_feb.iloc[11:35,:],'Mixing_ratio','Hour_wise','RMSE','201802' )


rmse_cor_feb=rmse_cor['2018-02-01':'2018-02-28']
plot_bars_rmse(rmse_cor_feb,'Mixing_Ratio','day_1','rmse',rmse_cor_feb.index.strftime('%Y%m')[0])

##################################################################################################################
mr_data_new_hour_feb=mr_data_new_hour['2018-05-31 13':'2018-06-29 12']
hourt = pd.to_timedelta(mr_data_new_hour_feb.index.hour,  unit='H')
mf=int(mr_data_new_hour_feb.shape[0]/24.0)
...
plot_bars_rmse_hw(mr_data_new_hour_feb.iloc[11:35,:],'Mixing_ratio','Hour_wise','RMSE','201806' )


rmse_cor_feb=rmse_cor['2018-06-01':'2018-06-29']
plot_bars_rmse(rmse_cor_feb,'Mixing_Ratio','day_1','rmse',rmse_cor_feb.index.strftime('%Y%m')[0])
